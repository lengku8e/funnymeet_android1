...
public class HDTimeListActivity extends Activity implements DownBack {
	...
	void init() {
		...
		((TextView) findViewById(R.id.type)).setText("主题");
		...
		((TextView) findViewById(R.id.type2)).setText("形式");
		...
		((TextView) findViewById(R.id.type4)).setText("评级");
	...
	ThemeSelectView themeView;

	void showThemeSelect() {
		if (themeView == null) {
			themeView = new ThemeSelectView(mActivity) {
				@Override
				public void onSelect(JSONObject theme1, JSONObject theme2,
					...
					HDTimeListActivity.this.requestData(1);
					hideThemeSelect();
				}
			};
		}
		if (themeView != null) {
			if (selectLayout2.indexOfChild(themeView.getMainView()) < 0) {
				selectLayout2.removeAllViews();
				selectLayout2.addView(themeView.getMainView());
