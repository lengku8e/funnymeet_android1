...
public class HDBrandSelectActivity extends Activity implements
	...
	void testData1() {
		try {
			...
			allViewDataList.add(new JSONObject("{'name':'消费类电子'}"));
			allViewDataList.add(new JSONObject("{'name':'奢饰品'}"));
			allViewDataList.add(new JSONObject("{'name':'日用品'}"));
			allViewDataList.add(new JSONObject("{'name':'汽车品牌'}"));
...
//		allView.updateCategory(allViewDataList);
		...
		task.addParam("method", "listBrandParentCategory");
...
//				allViewDataListItemContent);
		...
		Pdtask task = new Pdtask(this, back, Constants.SERVICE_HOST, null,
				RequestHelper.Type_DownJsonString, null, 0, true);
		task.addParam("method", "listBrandChildCategory");
	...
	public void onChildSelect(int index, JSONObject categoryJson,
			int indexChild, JSONObject childJson) {
		Intent mIntent = new Intent(mActivity, HDBrandListActivity.class);
		Bundle mBundle = new Bundle();
		HDBrandListActivity.HDInfo info = new HDBrandListActivity.HDInfo();
		info.parentJson = (new JSONObject()).toString();
		mBundle.putSerializable(HDBrandListActivity.HDInfo.key, info);
