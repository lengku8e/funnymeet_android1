...
public class LoginProblemModifyPassword extends BaseActivity {
	public final static int ID = LoginProblemModifyPassword.class.hashCode();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		doIntent();
		setContentView(R.layout.login_problem_modifypsw);
		init();
	...
	protected void init() {
		...
		finishbutton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (p1.length() < 6) {
					CustomDialog.createMsgDialog(mActivity, "密码不能小于6位", null)
							.show();
					...
					task.addParam("user_guid", user.optString("guid", null));// 页码
					task.addParam("password", p1);// 页码
					task.addParam("privileged",
							user.optString("privileged", null));// 页码
					task.addParam("long_session",
							user.optString("longSession", null));// 页码

					SOApplication.getDownLoadManager().startTask(task);
					showWait();
				} else {
					CustomDialog.createMsgDialog(mActivity, "帐号密码不一致", null)
							.show();
				}
			}
		});
